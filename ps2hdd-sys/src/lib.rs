...
mod tests {
    ...
    fn util_set_atad_device_path(path: &str) {
        ...
        let after_path = unsafe { std::ffi::CStr::from_ptr(atad_device_path.as_ptr()) }
            .to_str()
    ...
    fn util_iomanx_dopen_dread(path: &std::ffi::CString) -> Vec<iox_dirent_t> {
        ...
        assert_eq!(
            unsafe { iomanx_close(device_handle) },
            0,
            "iomanx_close failed"
        );
    ...
    fn format_list_partitions_and_write() {
        ...
        let demo_file_path = unsafe { std::ffi::CStr::from_ptr(atad_device_path.as_ptr()) }
            .to_str()
        ...
        assert_eq!(
            unsafe { _init_apa(0, std::ptr::null_mut()) },
            0,
            "_init_apa failed"
        );
        ...
        assert_eq!(
            unsafe { _init_pfs(0, std::ptr::null_mut()) },
            0,
            "_init_pfs failed"
        );
        ...
        // of a given partition within the `hdd0` structure.
        // assert_eq!(
        //     unsafe { _init_hdlfs(0, std::ptr::null_mut()) },
        //     0,
        //     "_init_hdlfs failed"
        // );
        ...
        let mkpart_path =
            std::ffi::CString::new("hdd0:TESTPART,,,128M,PFS").expect("couldn't convert string");
        ...
        if fd < 0 {
            let err = unsafe { std::ffi::CStr::from_ptr(libc::strerror(-fd)) }
                .to_str()
                .expect("could not convert error message a String");
            panic!("iomanx_open failed: {}, {}", fd, err);
        }

        assert_eq!(unsafe { iomanx_close(fd) }, 0, "iomanx_close failed");
        ...
        let partitions: Vec<_> = util_iomanx_dopen_dread(&hdd0_path)
            .iter()
            .map(|dirent| {
                unsafe { std::ffi::CStr::from_ptr(dirent.name.as_ptr()) }
                    .to_str()
        ...
        let testpart_path =
            std::ffi::CString::new("hdd0:TESTPART").expect("couldn't convert mount path string");
        let pfs0_root_path =
        ...
        if format_result < 0 {
            let err = unsafe { std::ffi::CStr::from_ptr(libc::strerror(-format_result)) }
                .to_str()
                .expect("could not convert error message a String");
        ...
        if mount_result < 0 {
            let err = unsafe { std::ffi::CStr::from_ptr(libc::strerror(-mount_result)) }
                .to_str()
                .expect("could not convert error message a String");
        ...
        let pfs0_testdir_path =
            std::ffi::CString::new("pfs0:/testdir").expect("couldn't convert mkdir path string");

        let mkdir_result = unsafe { iomanx_mkdir(pfs0_testdir_path.as_ptr(), 0o777) };

        if mkdir_result < 0 {
            let err = unsafe { std::ffi::CStr::from_ptr(libc::strerror(-mkdir_result)) }
                .to_str()
                .expect("could not convert error message a String");
        ...
        assert_eq!(
            unsafe { iomanx_umount(pfs0_root_path.as_ptr()) },
            0,
        ...
        assert_eq!(
            partitions,
            vec!["__mbr", "__net", "__system", "__sysconf", "__common"],
            "unexpected partition list"
        );
